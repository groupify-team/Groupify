...
export const usePlanLimits = () => {
   ...
   */
  const canPerformAction = useCallback((action, additionalData = {}) => {
    ...
    switch (action) {
      case 'create_trip':
        if (limits.trips !== 'unlimited' && additionalData.currentTripCount >= limits.trips) {
          return {
            allowed: false,
  ...
  const getUsageInfo = useCallback(() => {
    ...
    return {
      trips: {
        ...
        limit: limits.trips,
        percentage: limits.trips === 'unlimited' ? 0 : 
          Math.round(((usage.trips?.used || 0) / limits.trips) * 100),
        remaining: limits.trips === 'unlimited' ? 'unlimited' :
          Math.max(0, limits.trips - (usage.trips?.used || 0))
      },
      photos: {
        used: usage.photos.used,
        limit: limits.photosPerTrip,
        percentage: limits.photosPerTrip === 'unlimited' ? 0 :
          Math.round((usage.photos.used / limits.photosPerTrip) * 100),
        remaining: limits.photosPerTrip === 'unlimited' ? 'unlimited' :
          Math.max(0, limits.photosPerTrip - usage.photos.used)
      },
      storage: {
        used: usage.storage.used,
        usedFormatted: usage.storage.usedFormatted,
        limit: limits.storageGB === 'unlimited' ? 'unlimited' : limits.storageGB * 1024 * 1024 * 1024,
        limitFormatted: limits.storageGB === 'unlimited' ? 'unlimited' : `${limits.storageGB}GB`,
        percentage: limits.storageGB === 'unlimited' ? 0 : 
          Math.round((usage.storage.used / (limits.storageGB * 1024 * 1024 * 1024)) * 100),
        remaining: limits.storageGB === 'unlimited' ? 'unlimited' :
          Math.max(0, (limits.storageGB * 1024 * 1024 * 1024) - usage.storage.used),
        remainingFormatted: limits.storageGB === 'unlimited' ? 'unlimited' :
          subscriptionService.formatBytes(Math.max(0, (limits.storageGB * 1024 * 1024 * 1024) - usage.storage.used))
      }
    };
   ...
   */
  const showUpgradePrompt = useCallback((reason, options = {}) => {
    const {
    ...
    toast.error(reason, {
      ...
      action: {
        ...
        onClick: () => {
          ...
          console.log('Navigate to upgrade:', { reason, subscription: subscription?.plan });
   ...
   */
  const enforceLimit = useCallback((action, additionalData = {}, options = {}) => {
  ...
  const hasFeature = useCallback((feature) => {
    ...
    switch (feature) {
      ...
      case 'priority_support':
        return subscription.features.support === 'priority' || subscription.features.support === 'dedicated';
      default:
        return false;
    }
  }, [subscription, CORE_LIMITS]);
