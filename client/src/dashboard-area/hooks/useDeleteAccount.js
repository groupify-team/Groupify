...
export const useDeleteAccount = () => {
  ...
  const deleteAccount = useCallback(async (password, confirmationText) => {
    if (!currentUser) {
    ...
    try {
      ...
      const result = await DeleteAccountService.deleteAccount(currentUser, password);
      ...
      if (result.success) {
        ...
        navigate('/', { 
          replace: true, 
          state: { 
            message: 'Your account has been permanently deleted. Thank you for using Groupify!' 
          } 
        });
      ...
      if (err.code === 'auth/wrong-password') {
        ...
        setError('For security reasons, please log out and log back in, then try deleting your account again.');
