...
const SmartFaceScan = forwardRef(({ isOpen, onClose, onProfileCreated, onBack, onCameraStream }, ref) => {
  ...
  const captureCurrentStep = () => {
    ...
    canvas.toBlob(
      (blob) => {
        ...
        const file = new File([blob], `face-${stepData.id}-${Date.now()}.jpg`, {
          type: "image/jpeg",
        });
  ...
  useEffect(() => {
    return () => {
      console.log("🔄 SmartFaceScan component unmounting");
      stopCamera();
      capturedPhotos.forEach((photo) => {
        if (photo.url) {
          URL.revokeObjectURL(photo.url);
  ...
  useEffect(() => {
    if (!isOpen) {
      console.log("🚪 SmartFaceScan isOpen=false, stopping camera");
      stopCamera();
    }
  }, [isOpen]);
