...
const PhotoGallery = memo(
  ({
    ...
    // PERFORMANCE: Memoize the URL fixing function
    ...
    // PERFORMANCE: Memoize visible photos calculation
    const { visiblePhotos, overflowCount } = useMemo(() => {
      const visible = localPhotos.slice(0, gridCols - 1);
      const overflow = Math.max(0, localPhotos.length - visible.length);
      return { visiblePhotos: visible, overflowCount: overflow };
    }, [localPhotos, gridCols]);
    ...
    const handleFileUpload = useCallback(
      ...
      [tripId, currentUser?.uid, maxPhotos, localPhotos.length, onPhotoUploaded]
    );
