...
const EventStatistics = ({ event, photos, eventMembers }) => {
  ...
  const getPhotosPerDay = () => {
    if (!photos.length || !event.startDate) return 0;

    const eventstart = new Date(event.startDate);
    const now = new Date();
    const daysSinceStart = Math.max(
      1,
      Math.ceil((now - eventstart) / (1000 * 60 * 60 * 24))
    );
  ...
  const getPhotosPerMember = () => {
    if (!photos.length || !eventMembers.length) return 0;
    return Math.round((photos.length / eventMembers.length) * 10) / 10;
  };
    ...
    <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg rounded-xl shadow-lg border border-white/20 dark:border-gray-700/50 overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Insights about your event activity
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          {/* Photos per Member */}
          {photos.length > 0 && eventMembers.length > 0 && (
