...
const UserProfileModal = ({
  ...
  const handleConfirmedAction = async () => {
    switch (confirmAction) {
      ...
      case "invite-to-event":
        await handleAction(() => onInviteToEvent(user));
        break;
      case "leave-event":
        await handleAction(() =>
          onRemoveFromEvent(
  ...
  // Render role badge (event context only)
  const renderRoleBadge = () => {
    if (context !== "event" || !isEventMember) return null;
  ...
  return (
    <div
      className="modal-backdrop-standard event-modal-backdrop-enter p-3 sm:p-4"
      onClick={onClose}
    >
      <div
        className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm max-h-[95vh] overflow-hidden border border-gray-200 dark:border-gray-700 transform transition-all duration-300 event-modal-enter cursor-default modal-content-standard"
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex gap-2">
                  ...
                  {/* event Action Button */}
                  {context === "event" && !isEventMember && onInviteToEvent && (
                    <button
                      onClick={() => setConfirmAction("invite-to-event")}
                      disabled={loading}
          ...
          >
            ...
            >
              <h3 className="font-bold text-white text-lg flex items-center justify-center gap-2">
                ...
                {confirmAction === "kick" && "Remove from event"}
                {confirmAction === "remove-friend" && "Remove Friend"}
                {confirmAction === "invite-to-event" && "Invite to event"}
                {confirmAction === "leave-event" && "Leave event"}
            ...
            <div className="p-4 text-center">
              <p className="text-gray-700 dark:text-gray-300 text-sm mb-4 leading-relaxed">
                ...
                {confirmAction === "kick" &&
                  `Remove ${user.displayName || user.email} from this event?`}
                {confirmAction === "invite-to-event" &&
                  `Invite ${user.displayName || user.email} to this event?`}
                {confirmAction === "leave-event" &&
                  `Leave this event? You can be re-invited later.`}
                {confirmAction === "leave-admin" &&
