...
export const useFaceRecognition = (
  ...
  const saveResultsToStorage = (results, eventId) => {
    try {
      const dataToSave = {
        results,
        eventId,
      ...
      localStorage.setItem(
        `faceRecognition_${eventId}_${currentUserId}`,
  ...
  const loadResultsFromStorage = (eventId) => {
    try {
      ...
      if (saved) {
        ...
        if (data.eventId === eventId && data.userId === currentUserId) {
          return {
  ...
  useEffect(() => {
    if (currentUserId && canFilterByFace && eventId) {
      loadUserFaceProfile();

      // Load saved results on component mount
      const savedData = loadResultsFromStorage(eventId);
  ...
  }, [currentUserId, canFilterByFace, eventId]);
  ...
  const handleFindMyPhotos = async () => {
    if (!currentUserId || photos.length === 0) {
      toast.error("User ID or event photos missing");
      return;
    }
    ...
    try {
      ...
      if (matches.length > 0) {
        ...
        setFilterActive(true);
        saveResultsToStorage(matches, eventId);
  ...
  const handleClearScan = () => {
    ...
    try {
      localStorage.removeItem(`faceRecognition_${eventId}_${currentUserId}`);
    } catch (error) {
