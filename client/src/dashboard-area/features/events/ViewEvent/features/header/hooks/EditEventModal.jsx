...
const EditEventModal = ({
  ...
  const isAdmin =
    event?.admins?.includes(currentUser?.uid) ||
    event?.createdBy === currentUser?.uid;

  // Populate form with event data when event changes or modal opens
  useEffect(() => {
    if (event && isOpen) {
      setName(event.name || "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Notify parent component with updated event data
      if (onEventUpdated) {
        onEventUpdated({
          ...event,
          ...updatedeventData,
  ...
  const handleDelete = async () => {
    if (deleteConfirmText !== event.name) {
      setError(`Please type "${event.name}" exactly to confirm deletion`);
      return;
    }
    ...
    try {
      ...
      // IMPORTANT: Call onEventDeleted FIRST before navigation
      if (onEventDeleted) {
                ...
                <div className="flex items-center gap-1 sm:gap-2 md:gap-3">
                  ...
                  <div>
                    <h3 className="text-sm sm:text-base md:text-lg font-bold text-white">
                      {showDeleteConfirm ? "Delete event" : "Edit event"}
                    </h3>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="block text-xs sm:text-sm font-medium text-gray-800 dark:text-gray-200">
                      Type "{event.name}" to confirm deletion:
                    </label>
                    <input
                      type="text"
