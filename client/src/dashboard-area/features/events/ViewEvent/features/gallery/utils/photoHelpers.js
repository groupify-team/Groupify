...
export const checkPhotoLimit = (currentPhotoCount) => {
  return currentPhotoCount < MAX_PHOTOS_PER_EVENT;
};
...
export const getRemainingPhotoSlots = (currentPhotoCount) => {
  return Math.max(0, MAX_PHOTOS_PER_EVENT - currentPhotoCount);
};
...
export const getPhotoUsagePercentage = (currentPhotoCount) => {
  return Math.round((currentPhotoCount / MAX_PHOTOS_PER_EVENT) * 100);
};
...
export const validatePhotoUpload = (currentPhotoCount, newPhotosCount) => {
  ...
  const allowedCount = Math.max(0, MAX_PHOTOS_PER_EVENT - currentPhotoCount);
